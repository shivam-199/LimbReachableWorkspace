function [A] = structureMatrix4Red(M, L1, L2, Q1, Q2)
...
r2 = [(L1 * cosd(Q1)) (L1 * sind(Q1))];
r3 = [(L1 * cosd(Q1) + L2 * cosd(Q1 + Q2)) (L1 * sind(Q1) + L2 * sind(Q1 + Q2))];
r4 = [(L1 * cosd(Q1) + L2 * cosd(Q1 + Q2)) (L1 * sind(Q1) + L2 * sind(Q1 + Q2))];
...
dr4bydq1 = [(-L1 * sind(Q1) - L2 * sind(Q1 + Q2)) (L1 * cosd(Q1) + L2 * cosd(Q1 + Q2))];
dr1bydq2 = [0, 0];
dr2bydq2 = [0, 0];
dr3bydq2 = [(-L2 * sind(Q1 + Q2)) (L2 * cos(Q1 + Q2))];
dr4bydq2 = [(-L2 * sind(Q1 + Q2)) (L2 * cos(Q1 + Q2))];

%% Creating the Structure Matrix 2 * 3 (for 3 cables)
A = [dot(l1_cap, dr1bydq1) dot(l2_cap, dr2bydq1) dot(l3_cap, dr3bydq1) dot(l4_cap, dr4bydq1);
    dot(l1_cap, dr1bydq2) dot(l2_cap, dr2bydq2) dot(l3_cap, dr3bydq2) dot(l4_cap, dr4bydq2)];
end
